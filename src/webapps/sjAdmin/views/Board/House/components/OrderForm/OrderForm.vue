        ...
        <div class="modal" style="width: 80rem">
            ...
            <div class="content">
                <div class="box-table">
                    ...
                    <!-- 2 -->
                    <div class="no-scroll-bar" style="max-height: 40rem; overflow-y: auto">
                        <div class="row" v-for="(good, index) in pageModel.orderSource" :key="index">
                            <div class="column">{{ good.name }}</div>
                            <div class="column" style="width: 30rem">
                                <div class="column-input" v-for="(tag, countIndex) in good.plugList" :key="countIndex">
                                    <input type="number" v-model="tag.value" />
                                    <div class="sj-btn tag">{{ tag.name }}</div>
                                </div>
                            </div>
                            <div class="column">
                                <div class="column-input" v-for="(tag, countIndex) in good.countList" :key="countIndex">
                                    <input type="number" v-model="tag.value" />
                                    <div class="sj-btn tag">{{ tag.name }}</div>
                                </div>
                            </div>
                            ...
                            <div class="column">
                                <div class="sj-btn red tag" @click.stop="pageModel.orderSource.splice(index, 1)">
                                    删除
                                </div>
                            </div>
                        </div>
