  ...
  <div class="sj-board">
    ...
    <div class="side-right flex-y">
      ...
      <div class="con-nav flex-middle-y">
        <div>
          <span @click.stop="pageModel.sideShow = !pageModel.sideShow"
            >欢迎您：</span
          >
          <span class="sj-link" @click.stop="pageModel.reLogin">{{
            userInfo.phone || "请登录"
          }}</span>
          <span>，当前店铺：</span>
          <span class="sj-link" @click.stop="pageModel.toggleShopModal">{{
            shopInfo.name || "请选择您的店铺"
          }}</span>
          <span>，当前仓库：</span>
          <span class="sj-link" @click.stop="pageModel.toggleHouseModal">{{
            houseInfo.name || "请选择您的仓库"
          }}</span>
        </div>
      </div>
      <!-- 内容区 -->
      <div class="con-board">
        <div
          class="angle"
          @click.stop="pageModel.sideShow = !pageModel.sideShow"
        >
          <span
            class="fa"
            :class="{
    ...
    <div class="sj-modal-back" v-show="pageModel.modalLoginShow">
      <div class="modal" style="width: 20%">
        ...
        <div class="content">
          <input
            type="text"
            v-model="pageModel.userPhone"
            style="border: 1px solid red"
          />
          <input
            type="text"
            v-model="pageModel.userPassword"
            style="border: 1px solid red"
          />
        ...
        <div class="btns">
          ...
          <div
            class="sj-btn tip-on"
            @click.stop="pageModel.modalLoginShow = false"
          >
            取消
          </div>
        </div>
