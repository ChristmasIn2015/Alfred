<template>
    <div id="app">
        <div class="admin-board">
            <!-- ** -->
            <div class="side left-scroll-off" :class="{ 'left-scroll-on': pageModel.sideShow }" :style="{ width: !pageModel.sideShow ? '00%' : '' }">
                <div class="side-ad">
                    <span>DashBoard</span>
                </div>
                <div
            ...
            <div class="side-right flex-y">
                ...
                <div class="con-nav flex-middle-y">
                    <div>
                        ...
                        <span>，当前店铺：</span>
                        <span class="sj-link" @click.stop="pageModel.toggleShopModal">{{ shopInfo.name || '请选择您的店铺' }}</span>
                        <span>，当前仓库：</span>
                        <span class="sj-link" @click.stop="pageModel.toggleHouseModal">{{ houseInfo.name || '请选择您的仓库' }}</span>
                    </div>
                </div>
                <!-- 内容区 -->
                <div class="con-board">
                    <div class="angle" @click.stop="pageModel.sideShow = !pageModel.sideShow">
                        <span
                            class="fa"
                            :class="{
                                'fa-caret-left': pageModel.sideShow,
                                'fa-caret-right': !pageModel.sideShow,
                            }"
                        ></span>
                    </div>
                    <Employee v-if="pageModel.sideIndex === 0" ref="Shop" />
                    <House v-if="pageModel.sideIndex === 1" ref="House" />
                    <OrderForm v-if="pageModel.sideIndex === 2" ref="OrderForm" />
                    <OrderList v-if="pageModel.sideIndex === 3" ref="OrderList" />
                </div>
            </div>

            <!-- 登录弹窗 -->
            <div class="sj-modal-back" v-show="pageModel.modalloginModal">
                <div class="modal" style="width: 20%">
                    ...
                    <div class="content">
                        <div class="form-line">
                            <input type="number" v-model="pageModel.userPhone" placeholder="请输入手机号" />
                        </div>
                        <div class="form-line">
            ...
            <!-- 店铺弹窗 -->
            <div class="sj-modal-right right-scroll-off" :class="{ 'right-scroll-on': pageModel.shopModalShow }">
                ...
                <div class="right">
                    ...
                    <div class="card">
                        ...
                        <div
                            class="sj-btn tip-on"
                            v-for="(shop, index) in pageModel.officeList"
                            :key="index"
                            :class="{ tip: shopInfo._id === shop._id }"
                            @click.stop="pageModel.pickShop(shop)"
                        >
            ...
            <!-- 仓库弹窗 -->
            <div class="sj-modal-right right-scroll-off" :class="{ 'right-scroll-on': pageModel.houseModalShow }">
...
<script>
    ...
    import OrderList from '../Board/OrderList/OrderList.vue'
    import OrderForm from '../Board/OrderForm/OrderForm.vue'
