...
export function getCalendarMatrix(startMills, endMills) {
    ...
    for (let i = 0; i <= dayCount; i++) {
        ...
        if (monthValue.length === 0) {
            let weekDay =
                new Date(`${yearKey}-${monthKey}-${dayString}`).getDay() - 1
        ...
        for (let i = 0; i < vacationList.length; i++) {
            ...
            if (
                dayCharge.year === yearKey &&
                dayCharge.month === monthKey &&
                dayCharge.day === dayString
            ) {
                type = dayCharge.type
                break // 跳出这个循环
            }
        ...
        let dateEntry = {
            ...
            past: nowDayMill < todayMill,
            today:
                getYYMMDD(todayMill) === `${yearKey}-${monthKey}-${dayString}`,
            type: type,
        }
        calendarMatrix
            .get(yearKey)
