...
export default class Tag {
    ...
    async addTag(request, response, Cabin) {
        ...
        let tag = await Cabin.Tag.create(model)
        ...
        if (goodId) {
            ...
            // 3.2 新增全局标签(关系)
            await Cabin.GoodTag.create({ goodId: null, tagId: tag._id, tagType: -1 })
        }
    }
    ...
    async getPlugList(request, response, Cabin) {
        ...
        let list = await Cabin.GoodTag.query(query)
        let tagList = []
        for (let i = 0; i < list.length; i++) {
            let tag = await Cabin.Tag.get({ _id: list[i].tagId })
