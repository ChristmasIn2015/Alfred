...
export default class Order {
    ...
    async addOrder(request, response) {
        ...
        for (let index = 0; index < list.length; index++) {
            ...
            for (let i = 0; i < good.outCountList.length; i++) {
                ...
                let targetTag = await $db.Tag.get(query)
                await $db.Tag.update(query, { value: Number(targetTag.value) - Number(counter.value) })
            }
            // 入库准备
        ...
        let order = await $db.Order.create(model)
        return order
    }
    ...
    async getOrderList(request, response) {
        ...
        // 2.
        let orderList = await $db.Order.query({ byHouseId: request.body.houseId })
        return orderList
    }
    ...
    async updateOrder(request, response) {
        ...
        // 2.找到这条订单
        let order = await $db.Order.get({ _id: request.body.orderId })
        ...
        await $db.Order.update({ _id: request.body.orderId }, updater)
    }

    // 修改订单状态
    @Response()
    async changeOrderStatus(request, response) {
        // 1.
        let user = await Cabin.userCharge(request, response)
    ...
    // 商品撤回 库存还原
    @Response()
    async clearOrderGood(request, response) {
        // 1.
        let user = await Cabin.userCharge(request, response)
        ...
        if (order.goodStatus === 11) {
            ...
            // 实际发货清空
            await $db.Order.update({ _id: request.body.orderId }, { goodList: [], goodStatus: 10 })
        } else {
