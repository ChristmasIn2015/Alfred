...
export default class User {
    ...
    async login(request, response, Cabin) {
        ...
        let user = await Cabin.User.get(userQueryKey)

        // 2.如果用户不存在则新增用户
        if (!user) user = await Cabin.User.create(Object.assign({}, userModel))
        ...
        await Cabin.User.update(userQueryKey, updateModel) // User

        // 5.再次获取这个用户
        user = await Cabin.User.get(userQueryKey)
    ...
    async getUserInfo(request, response, Cabin) {
        ...
        if (!request.header('authorization')) throw new Error('获取用户信息失败, 请重新登录')
        let userPhone = this.#getTokenInfo(request.header('authorization')).phone
        let user = await Cabin.User.get({ phone: userPhone }) // User
