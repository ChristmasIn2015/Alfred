...
export default class Good {
    ...
    //
    @Response()
    async getGoodList(request, response) {
        // 1.
        let user = await Cabin.userCharge(request, response)

        // 2.取得所有商品
        let goodList = await $db.Good.query({ byHouseId: request.body.byHouseId })
        ...
        for (let i = 0; i < goodList.length; i++) {
            ...
            // * 查询并配置所有规格标签
            let plugList = await $db.GoodTag.query({ goodId: good._id, tagType: 0 })
            // console.log({ goodId: good._id, tagType: 0 })
            let newPlugList = []
            for (let i = 0; i < plugList.length; i++) {
                let tag = await $db.Tag.get({ _id: plugList[i].tagId })
                if (tag) newPlugList.push(tag)
            }

            // * 查询并配置所有单位标签
            let countList = await $db.GoodTag.query({ goodId: good._id, tagType: 1 })
            let newCountList = []
            for (let i = 0; i < countList.length; i++) {
                let tag = await $db.Tag.get({ _id: countList[i].tagId })
